...
def evaluate(human_labeled_file,pred_file):
    ...
    macro_f1 = f1_score(human_labeled_senti, pred_senti, labels=senti_labels, average='macro')
    micro_f1 = f1_score(human_labeled_senti, pred_senti, labels=senti_labels, average='micro')
    # 输出 overall accuracy, Macro-F1, Micro-F1
    print(f"accuracy_score: {accuracy}")
    print(f"macro: {macro_f1}")
    print(f"micro: {micro_f1}")
    result.extend([accuracy, macro_f1, micro_f1])
    # 计算每个类别的precision, recall, F1
    precision = precision_score(human_labeled_senti, pred_senti, labels=senti_labels, average=None)
    recall = recall_score(human_labeled_senti, pred_senti, labels=senti_labels, average=None)
    f1 = f1_score(human_labeled_senti, pred_senti, labels=senti_labels, average=None)
...
def draw_confusion_matrix(human_labeled_file,pred_file):
    ...
    # 使用热力图绘制混淆矩阵
    sns.heatmap(cm_percentage, annot=True, fmt=".1%", cmap="Blues", xticklabels=senti_labels, yticklabels=senti_labels, annot_kws={"size": 33})
...
if __name__ == '__main__':
    ...
    pred_file = f'ChatGPT/outputs/{target}_formated_p6.11.csv'
